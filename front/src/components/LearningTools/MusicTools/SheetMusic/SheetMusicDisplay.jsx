...
const SheetMusicDisplay = ({ notes }) => {
  ...
  useEffect(() => {
    ...
    try {
      vexNotes = notes.map(note => {
        ...
        const pitch = /^[a-gA-G](#|b)?\/[0-9]$/.test(note) ? note : 'c/4';
        return new VF.StaveNote({ clef: 'treble', keys: [pitch], duration: 'q' });
      });
    } catch (error) {
  ...
  return <div className="sheet-music-display" ref={containerRef}></div>;
};
